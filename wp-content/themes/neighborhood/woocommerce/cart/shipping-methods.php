...
if ( $available_methods ) {
	...
	// Show radio buttons for methods
		...
		foreach ( $available_methods as $method ):
			...
			echo '<option value="' . esc_attr( $method->id ) . '" ' . selected( $method->id, $woocommerce->session->chosen_shipping_method, false ) . '>' . ( esc_attr( $method->id ) == 'flat_rate' ? wp_kses_post( $method->full_label ) :  wp_kses_post( $method->label ) ) . '</option>';
			//var_dump($method);
		endforeach;
/*
		if($band == false):
