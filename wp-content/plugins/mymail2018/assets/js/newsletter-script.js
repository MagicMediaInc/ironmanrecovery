			...
			if (!_disabled) {
				...
				_iframe.contents().find('body')
					.on('mouseover', '*', function(e){
						...
						var properties = ['color','backgroundColor','background', 'borderColor'],
							color, colors = [], bg;

						if(!(/(TD|TBODY|TR|TABLE)/.test(type))){
							e.stopPropagation();
						}else{
							if(bg = e.target.getAttribute('bgcolor')) colors.push(bg.substr(1));
						}
						_colorpickers.removeClass('affected');


						for (var i = properties.length - 1; i >= 0; i--) {
							color = window.getComputedStyle(e.target, null)[properties[i]].match(/rgba?\((\d+), (\d+), (\d+)\)/);
							if(color) colors.push(_rgbToHex(parseInt(color[1], 10),parseInt(color[2], 10),parseInt(color[3], 10), false));
						};

						for (var i = colors.length - 1; i >= 0; i--) {
							$('#mymail-color-'+colors[i]).addClass('affected');
						};
						
					})
					.find('.mymail-loading').removeClass('mymail-loading');
			} else {}
