...
<?php elseif('export' == $currentpage && current_user_can('mymail_export_subscribers')) :?>
		...
		<div class="step1">
			...
			foreach($lists as $list){
				?>
				<li><label><input type="checkbox" name="lists[]" value="<?php echo $list->ID?>" data-count="<?php echo $list->subscribers?>" checked> <?php echo $list->name. ' <span class="count small">('.number_format_i18n($list->subscribers).' '.__('total', 'mymail').')</span>'?></label></li>
				<?php 
			}
			*/
			?>
