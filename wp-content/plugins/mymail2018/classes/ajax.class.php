...
class mymail_ajax {
	...
	private function forward_message() {
		...
		$return['success'] = $mail->send_notification(nl2br($data['message']) . "<br><br>" . $data['url'], '', array('notification' => sprintf(__('%1$s is forwarding this mail to you via %2$s', 'mymail'), $data['sendername'].' (<a href="mailto:'.esc_attr($data['sender']).'">'.esc_attr($data['sender']).'</a>)', '<a href="'.get_bloginfo('url').'">'.get_bloginfo('name').'</a>' )));
