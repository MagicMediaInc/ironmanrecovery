...
class mymail_form {
	...
	public function form($form_id = 0, $tabindex = 100, $classes = '') {
		...
		$form_id = (isset($forms[$form_id])) ? $form_id : 0;
		$form = $forms[$form_id];
		...
		$html .= '<form action="'.admin_url('admin-ajax.php').'" method="post" class="mymail-form mymail-form-submit mymail-form-'.$form_id.' '.(mymail_option('ajax_form') && !$backend ? 'mymail-ajax-form ' : '').''.esc_attr($classes).'">';
		...
		if(!$backend){
			...
			$html .= '<input name="_redirect" type="hidden" value="'.$redirect.'">';
			$html .= '<input name="_referer" type="hidden" value="'.$referer.'">';
			...
			foreach($form['order'] as $field){
				...
				switch($field){
					...
					case 'email':
						...
						$fields['email'] .= '<input id="mymail-email-'.$form_id.'" name="userdata[email]" type="text" value="'.(isset($this->object['userdata']['email']) ? $this->object['userdata']['email'] : '').'"'.($inline ? ' placeholder="'.$label.($asterisk ? ' *' : '').'"' : '').' class="input mymail-email required" tabindex="'.($tabindex++).'">';
						$fields['email'] .= '</div>';
					...
					case 'firstname':
						...
						$fields['firstname'] .= '<input id="mymail-firstname-'.$form_id.'" name="userdata[firstname]" type="text" value="'.(isset($this->object['userdata']['firstname']) ? $this->object['userdata']['firstname'] : '').'"'.($inline ? ' placeholder="'.$label.($required && $asterisk ? ' *' : '').'"' : '').' class="input mymail-firstname'.($required ? ' required' : '').'" tabindex="'.($tabindex++).'">';
					...
					case 'lastname':
						...
						$fields['lastname'] .= '<input id="mymail-lastname-'.$form_id.'" name="userdata[lastname]" type="text" value="'.(isset($this->object['userdata']['lastname']) ? $this->object['userdata']['lastname'] : '').'"'.($inline ? ' placeholder="'.$label.($required && $asterisk ? ' *' : '').'"' : '').' class="input mymail-lastname'.($required ? ' required' : '').'" tabindex="'.($tabindex++).'">';
						$fields['lastname'] .= '</div>';
					...
					switch($data['type']){
						...
						$i = 0;
						foreach($data['values'] as $v){
							$fields[$field] .= '<li><label><input id="mymail-'.$field.'-'.$form_id.'-'.($i++).'" name="userdata['.$field.']" type="radio" value="'.$v.'" class="radio mymail-'.$field.''.($required ? ' required' : '').'" tabindex="'.($tabindex++).'" '.(isset($data['default']) ? checked($data['default'], (isset($this->object['userdata'][$field]) ? $this->object['userdata'][$field] : $v), false) : '').'> '.$v.'</label></li>';
						}
						$fields[$field] .= '</ul>';
							break;
						...
						case 'checkbox':
							...
							$fields[$field] .= '<input id="mymail-'.$field.'-'.$form_id.'" name="userdata['.$field.']" type="checkbox" value="1" '.((isset($this->object['userdata'][$field]) || isset($data['default']) ) ? ' checked' : '').' class="mymail-'.$field.''.($required ? ' required' : '').'" tabindex="'.($tabindex++).'"> ';
							$fields[$field] .= ' '.$label;
							if ($required && $asterisk) $fields[$field] .= ' <span class="required">*</span>';
							$fields[$field] .= '</label>';
						...
						case 'date':
							...
							$fields[$field] .= '<input id="mymail-'.$field.'-'.$form_id.'" name="userdata['.$field.']" type="text" value="'.(isset($this->object['userdata'][$field]) ? $this->object['userdata'][$field] : '').'"'.($inline ? ' placeholder="'.$label.($required && $asterisk ? ' *' : '').'"' : '').' class="input input-date mymail-'.$field.''.($required ? ' required' : '').'" tabindex="'.($tabindex++).'">';
							
							break;
						default:
							$fields[$field] .= '<input id="mymail-'.$field.'-'.$form_id.'" name="userdata['.$field.']" type="text" value="'.(isset($this->object['userdata'][$field]) ? $this->object['userdata'][$field] : '').'"'.($inline ? ' placeholder="'.$label.($required && $asterisk ? ' *' : '').'"' : '').' class="input mymail-'.$field.''.($required ? ' required' : '').'" tabindex="'.($tabindex++).'">';
					}
	...
	public function profile($hash = NULL, $tabindex = 100, $classes = '') {
		...
		$html .= '<form action="'.admin_url('admin-ajax.php').'" method="post" class="mymail-form mymail-form-submit mymail-form-'.$form_id.' '.(mymail_option('ajax_form') ? 'mymail-ajax-form ' : '').''.esc_attr($classes).'">';
		...
		$html .= '<input name="_redirect" type="hidden" value="'.$referer.'">';
		$html .= '<input name="_referer" type="hidden" value="'.$referer.'">';
		...
		//foreach($all_fields as $field){
			...
			switch($field){
				...
				case 'email':
					...
					$fields['email'] = '<div class="mymail-wrapper mymail-email-wrapper'.(isset($this->object['errors']['email']) ? ' error' : '').'">';
					if(!$inline) $fields['email'] .= '<label for="mymail-email-'.$form_id.'">'.$label.' '.($asterisk ? '<span class="required">*</span>' : '').'</label>';
					$fields['email'] .= '<input id="mymail-email-'.$form_id.'" name="userdata[email]" type="text" value="'.$subscriber->email.'"'.($inline ? ' placeholder="'.$label.($asterisk ? ' *' : '').'"' : '').' class="input mymail-email required" tabindex="'.($tabindex++).'">';
					$fields['email'] .= '</div>';
				...
				case 'firstname':
					...
					$fields['firstname'] = '<div class="mymail-wrapper mymail-firstname-wrapper'.(isset($this->object['errors']['firstname']) ? ' error' : '').'">';
					if(!$inline) $fields['firstname'] .= '<label for="mymail-firstname-'.$form_id.'">'.$label.($required && $asterisk ? ' <span class="required">*</span>' : '').'</label>';
					$fields['firstname'] .= '<input id="mymail-firstname-'.$form_id.'" name="userdata[firstname]" type="text" value="'.$subscriber->firstname.'"'.($inline ? ' placeholder="'.$label.($required && $asterisk ? ' *' : '').'"' : '').' class="input mymail-firstname'.($required ? ' required' : '').'" tabindex="'.($tabindex++).'">';
					$fields['firstname'] .= '</div>';
				...
				case 'lastname':
					...
					$fields['lastname'] = '<div class="mymail-wrapper mymail-lastname-wrapper'.(isset($this->object['errors']['lastname']) ? ' error' : '').'">';
					if(!$inline) $fields['lastname'] .= '<label for="mymail-lastname-'.$form_id.'">'.$label.($required && $asterisk ? ' <span class="required">*</span>' : '').'</label>';
					$fields['lastname'] .= '<input id="mymail-lastname-'.$form_id.'" name="userdata[lastname]" type="text" value="'.$subscriber->lastname.'"'.($inline ? ' placeholder="'.$label.($required && $asterisk ? ' *' : '').'"' : '').' class="input mymail-lastname'.($required ? ' required' : '').'" tabindex="'.($tabindex++).'">';
					$fields['lastname'] .= '</div>';
				...
				switch($data['type']){
					...
					case 'date':
						...
						$fields[$field] .= '<input id="mymail-'.$field.'-'.$form_id.'" name="userdata['.$field.']" type="text" value="'.$subscriber->{$field}.'"'.($inline ? ' placeholder="'.$label.($required && $asterisk ? ' *' : '').'"' : '').' class="input input-date mymail-'.$field.''.($required ? ' required' : '').'" tabindex="'.($tabindex++).'">';

						break;
						
					default:
						$fields[$field] .= '<input id="mymail-'.$field.'-'.$form_id.'" name="userdata['.$field.']" type="text" value="'.$subscriber->{$field}.'"'.($inline ? ' placeholder="'.$label.($required && $asterisk ? ' *' : '').'"' : '').' class="input mymail-'.$field.''.($required ? ' required' : '').'" tabindex="'.($tabindex++).'">';
				}
		...
		$label = mymail_text('profilebutton', __('Update Profile', 'mymail'));
	...
	public function unsubscribe_form($hash = '', $campaignid = '', $tabindex = 100, $classes = '') {
		...
		$html .= '<input name="_redirect" type="hidden" value="'.$_SERVER['REQUEST_URI'].'">';
	...
	public function handle_submission( ) {
		...
		if(empty($this->object['lists']) && isset($form['userschoice']))
			$this->object['errors']['lists'] = __('Select at least one list', 'mymail');
		
		if ($this->valid()) {
			$email = $this->object['userdata']['email'];

			$result = mymail('subscribers')->add(wp_parse_args(array(
				'signup' => $now,
				...
				if($result->get_error_code() == 'email_exists'){
					...
					if($exists = mymail('subscribers')->get_by_mail($this->object['userdata']['email'])){
						...
						if($exists->status == 0){
							...
							mymail('subscribers')->send_confirmations($exists->ID, true);
	...
	public function profile_update( ) {
		...
		if ($this->valid()) {
			...
			$result = mymail('subscribers')->update($this->object['userdata'], true, true);
