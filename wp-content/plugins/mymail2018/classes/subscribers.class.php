...
class mymail_subscribers {
	...
	public function sync_subscriber( $subscriber_id ) {
		...
		foreach($synclist as $field => $meta){
			//if(!isset($subscriber->{$field})) continue;
	...
	public function update($entry, $overwrite = true, $merge = false, $subscriber_notification = false) {
		...
		if(false !== $wpdb->query($sql)){
			...
			$subscriber_id = !empty($wpdb->insert_id) ? $wpdb->insert_id : $data['ID'];
	...
	public function add_from_wp_user($user_id, $userdata = array()) {
		...
		if(!isset($userdata['status'])){
			...
			if($is_register){
				$userdata['status'] = $double_opt_in ? 0 : 1;
			}else{
	...
	public function add_custom_value($subscriber_id, $key, $value = NULL, $clear = false) {
		...
		foreach($fields as $key => $value){
			//if(!isset($customfields[$key])) continue;
	...
	public function assign_lists($subscriber_ids, $lists, $remove_old = false) {
		...
		if($remove_old) $this->unassign_lists($subscriber_ids);
		...
		foreach($subscriber_ids as $subscriber_id){
			...
			$sql = "INSERT INTO {$wpdb->prefix}mymail_lists_subscribers
			(list_id, subscriber_id, added) VALUES ";
		...
		return $count == count($subscriber_ids);

	}

	public function unassign_lists($subscriber_ids, $lists = NULL) {
	...
	public function get_timeoffset_timestamps($subscriber_ids, $timestamps = NULL){
		...
		if(!is_array($timestamps)){
			...
			$sql = "SELECT IF(b.meta_value IS NULL,$timestamps,b.meta_value*3600+$timestamps) AS timestamp FROM {$wpdb->prefix}mymail_subscribers AS a LEFT JOIN {$wpdb->prefix}mymail_subscriber_meta AS b ON a.ID = b.subscriber_id AND meta_key = 'timeoffset' WHERE a.ID IN (".implode(',', $subscriber_ids).") ORDER BY a.ID ASC";

			return $wpdb->get_col($sql);

		}else{

			$sql = "SELECT a.ID, IF(b.meta_value IS NULL,0,b.meta_value*3600) AS offset FROM {$wpdb->prefix}mymail_subscribers AS a LEFT JOIN {$wpdb->prefix}mymail_subscriber_meta AS b ON a.ID = b.subscriber_id AND meta_key = 'timeoffset' WHERE a.ID IN (".implode(',', $subscriber_ids).") ORDER BY a.ID ASC";
	...
	public function change_status($subscriber_ids, $new_status, $silent = false) {
		...
		foreach($subscriber_ids as $subscriber_id){
			...
			if ($wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}mymail_subscribers SET status = %d WHERE ID = %d", $new_status, $subscriber->ID))){
