...
class mymail_campaigns {
	...
	public function post_edit_assets(){
		...
		wp_localize_script('mymail-script', 'mymaildata', array(
			...
			'datefields' => mymail()->get_custom_date_fields(true),
		));
	...
	public function get_subscribers($id = NULL, $statuses = NULL, $return_ids = false, $notsentonly = false){
		...
		return $this->get_subscribers_by_lists($lists, $conditions, $statuses, $return_ids, $notsentonly ? $id : false);
	}


	public function get_subscribers_by_lists($lists = false, $conditions = NULL, $statuses = NULL, $return_ids = false, $notsentonlyfromid = false){
		
		global $wpdb;

		$cache_key = 's_'.md5(serialize(array($lists,$conditions,$statuses,$return_ids,$notsentonlyfromid)));
		...
		if($notsentonlyfromid)
			$sql .= " LEFT JOIN {$wpdb->prefix}mymail_actions AS b ON a.ID = b.subscriber_id AND b.campaign_id = ".intval($notsentonlyfromid)." AND b.type = 1";
		...
		if($return_ids) $sql .= " GROUP BY a.ID ORDER BY a.ID ASC";

		$subscribers = ($return_ids) ? $wpdb->get_col($sql) : $wpdb->get_var($sql);
	...
	public function get_unsent_subscribers($id = NULL, $statuses = NULL, $return_ids = false){
		return $this->get_subscribers($id, $statuses, $return_ids, true);
	}
	...
	public function get_geo_data($id){
		...
		foreach($result as $row){
			...
			if(!$row->coords){
				//if(!isset($geo_data[$geo[0]][0])) $geo_data[$geo[0]][0] = 0;
				$geo_data[$geo[0]][0][3]++;
	...
	public function get_sql_join_by_condition($conditions, $tablealias = 'data') {
	...
	public function get_sql_by_condition($conditions, $tablealias = 'a') {
		...
		$timefields = array_merge( array('added', 'updated', 'signup', 'confirm'), $custom_date_fields);
	...
	public function send( $metadata, $userdata, $track = true, $force = false, $log = true ) {
		...
		if($track){
			...
			//replace links
			$placeholder->replace_links($links, $metadata->hash);
