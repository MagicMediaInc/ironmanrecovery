...
*/
		...
		if(!empty($mymail_mytags) && $this->replace_custom){
			...
			foreach($mymail_mytags as $tag => $replacecallback){
				...
				if($count = preg_match_all( '#\{'.$tag.':?([^\}|]+)?\|?([^\}]+)?\}#i', $this->content, $hits_fallback )){
	...
	public function replace_links($links = array(), $hash = '') {
		
		$used = array();
		
		$new_structure = mymail('helper')->using_permalinks();
		$base = mymail()->get_base_link($this->campaignID);
		...
		foreach ( $links as $link ) {
			...
			if($new_structure){
				$replace = $base . '/' . $hash . '/' . rtrim(strtr(base64_encode($link), '+/', '-_'), '=');
				...
				$target = str_replace( array('%7B', '%7D') , array( '{', '}' ), urlencode( $link ) );
			...
			// if(!isset($used[$link])){
			// 	$replace = '"'.$base . '&k=' . $hash . '&t=' . $target .'&c=0&s=' . $secure . '"';
			// 	$used[$link] = 1;
			// }else{
			// 	$replace = '"'.$base . '&k=' . $hash . '&t=' . $target .'&c=' . ($used[$link]++) . '&s=' . $secure . '"';
			// }
			
			//$replace = '"'.$base . '&k=' . $hash . '&t=' . $target .'&s=' . $secure . '"';

			//$replace = '"'.$base . '/' . $hash . '/' . base64_encode($target) .'&s=' . $secure . '"';

			if (($pos = strpos($this->content, $search)) !== false) {
	...
	private function get_last_post( $offset = 0, $post_type = 'post', $term_ids = false ) {
		...
		$post = wp_cache_get( 'get_last_post_'.$offset.'_'.$post_type, 'mymail' );
		...
		wp_cache_set( 'get_last_post_'.$offset.'_'.$post_type, $post, 'mymail' );
