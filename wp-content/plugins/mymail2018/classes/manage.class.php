...
class mymail_manage {
	...
	public function ajax_import_subscribers_upload_handler() {
		...
		if(isset($_FILES['async-upload'])){
			...
			$sql = "SELECT u.user_email, meta_role.meta_value AS '_role', meta_firstname.meta_value AS 'firstname', meta_lastname.meta_value AS 'lastname', u.display_name, u.user_nicename, u.user_registered";
			...
			foreach($users as $user){
				...
				if(!isset($user->_role)) continue;

				if(!array_intersect(array_keys(unserialize($user->_role)), $roles)) continue;
