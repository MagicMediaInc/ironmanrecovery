...
class mymail_queue {
	...
	public function init() {
		...
		add_action('mymail_cron', array( &$this, 'update'));
		add_action('mymail_cron', array( &$this, 'autoresponder_timebased'));
		add_action('mymail_cron', array( &$this, 'autoresponder_usertime'));
		add_action('mymail_cron', array( &$this, 'autoresponder'));
		add_action('mymail_cron', array( &$this, 'cleanup'));
		//add_action('mymail_cron', array( &$this, 'autoresponder'));
		
		add_action('mymail_cron_worker', array( &$this, 'update'));
		add_action('mymail_cron_worker', array( &$this, 'autoresponder'));
		add_action('mymail_cron_worker', array( &$this, 'progress'));
		add_action('mymail_cron_worker', array( &$this, 'finish_campaigns'));
		
		add_action('mymail_update_queue', array( &$this, 'autoresponder'));
		add_action('mymail_update_queue', array( &$this, 'update'));
	...
	public function remove($campaign_id, $subscribers = NULL, $requeued = false) {

		global $wpdb;

		$sql = "DELETE a FROM {$wpdb->prefix}mymail_queue AS a WHERE a.campaign_id = %d";
	...
	public function clear($campaign_id, $subscribers = array()) {
		...
		$sql = "DELETE a FROM {$wpdb->prefix}mymail_queue AS a WHERE a.campaign_id = %d AND a.sent = 0 AND a.subscriber_id NOT IN (".implode(',', $subscribers).")";

		return false !== $wpdb->query($wpdb->prepare($sql, $campaign_id));
	...
	public function update() {
		...
		$campaigns = mymail('campaigns')->get_campaigns(array('post_status' => array('queued', 'active')));
		...
		foreach($campaigns as $campaign){
			...
			//only which unsent subscribers
			$subscribers = mymail('campaigns')->get_unsent_subscribers($campaign->ID, array(1), true);

			if(empty($subscribers)) continue;

			//get users timeoffsets
			if(mymail('campaigns')->meta($campaign->ID, 'timezone')){
				
				$timestamp = mymail('subscribers')->get_timeoffset_timestamps($subscribers, $timestamp);
				$smallest = min($timestamp);

				if($campaign->post_status == 'queued' && $smallest-$now <= 0){
					mymail('campaigns')->change_status($campaign, 'active');
				}

			//if queue and timestamp is now or later and no timezones are used
			}else{

				if($campaign->post_status == 'queued' && $timestamp-$now <= 0){
					mymail('campaigns')->change_status($campaign, 'active');
				}
			}
	...
	public function autoresponder_timebased($campaign_id = NULL) {
		...
		foreach($campaigns as $campaign){
			...
			if('mymail_autoresponder_timebased' == $autoresponder_meta['action']){
				...
				if($starttime-$now > 3600 && !isset($meta['timezone']) 
	...
	public function autoresponder_usertime($campaign_id = NULL) {
		...
		foreach($campaigns as $campaign){
			...
			if('mymail_autoresponder_usertime' == $autoresponder_meta['action']){
				...
				//$subscriber_ids = mymail('campaigns')->get_subscribers($campaign->ID, NULL, true);
				...
				foreach($chunks as $ids){
					...
					$sql = $wpdb->prepare("SELECT a.ID, a.email, b.meta_value AS date, UNIX_TIMESTAMP(b.meta_value)+".intval(0)." AS timestamp FROM {$wpdb->prefix}mymail_subscribers AS a LEFT JOIN {$wpdb->prefix}mymail_subscriber_fields AS b ON a.ID = b.subscriber_id WHERE b.meta_key = %s AND b.meta_value != '' AND a.status = 1", $autoresponder_meta['uservalue']);
					...
					foreach($subscribers as $subscriber){
						...
						$nextdate = mymail('helper')->get_next_date_in_future($subscriber->timestamp + $send_offset, $autoresponder_meta['useramount'], $autoresponder_meta['userunit']);
	...
	public function finish_campaigns() {
		...
		foreach($ids as $id){
			...
			$errors = mymail('campaigns')->get_errors($id);

			if(mymail('campaigns')->change_status(mymail('campaigns')->get($id), 'finished')){
				mymail('campaigns')->update_meta($id, 'sent', $sent);
				mymail('campaigns')->update_meta($id, 'errors', $errors);
				mymail('campaigns')->update_meta($id, 'finished', time());

				$parent_id = mymail('campaigns')->meta($id, 'parent_id');

				if($parent_id = mymail('campaigns')->meta($id, 'parent_id')){
					$parent_sent = mymail('campaigns')->meta($parent_id, 'sent');
					$parent_errors = mymail('campaigns')->meta($parent_id, 'errors');

					mymail('campaigns')->update_meta($parent_id, 'sent', $parent_sent+$sent);
					mymail('campaigns')->update_meta($parent_id, 'errors', $parent_errors+$errors);
				}
	...
	public function progress($cron_used = false) {
		...
		if($to_send){
			...
			//subscriber status us 1 (subscribed) or ignore_status
			...
			foreach($result as $i => $data){
				...
				if($metadata->campaign_id){
					...
					if(!$wpdb->get_var($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE ID = %d AND post_status IN ('active', 'finished', 'autoresponder') AND post_type = 'newsletter'", $metadata->campaign_id)) && !$force){
				...
				//error
					...
					//campaign_error
						...
						if($campaign->post_status == 'autoresponder'){
							mymail_notice(sprintf(__( 'Autoresponder %1$s has caused sending error: %2$s', 'mymail'), '<a href="post.php?post='.$campaign->ID.'&action=edit"><strong>'.$campaign->post_title.'</strong></a>', '<strong>'.implode('',$result->get_error_messages())).'</strong>', 'updated', false, 'camp_error_'.$campaign->ID);
