...
if (!function_exists('cmshowcase_get_featured_img')) {
	...
	function cmshowcase_get_featured_img( $id, $size = 'thumbnail', $email = '', $use_default = true, $default = '', $dsize = array(), $gravatar = true) {
		...
		//If we want to use profile image of existing user
		//still in dev
		$use_profile = false;
		if($use_profile && $email!='') {
			
			$get_avatar = get_avatar( $email, $dsize[0], $default );
			...
			foreach ($thumbs as $thumb) {
				...
				if($size == $thumb) {
					...
					if( in_array( $thumb, array( 'thumbnail', 'medium', 'large' ) ) ){
		 				...
		 				$img_array['height'] = get_option( $thumb . '_size_h' );
		 			} 
		...
		if($gravatar) {
			...
			if(get_post_thumbnail_id( $id ) == false && $email != '') {

				$avatar = cmshowcase_gravatar_exists($email,$img_array['width']);
				if($avatar!=false) {

					$img_array['src'] = $avatar;	
		...
		if(!$gravatar) {
			...
			if(get_post_thumbnail_id( $id ) == false ) {
